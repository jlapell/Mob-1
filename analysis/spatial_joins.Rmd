...
```{r}
# This function from the sf package reads GeoJSON files
...
# A lot of useful stuff in this code chunk:
# messages and warnings off
# as_date
# separate function
# select all *but* the fields provided with -
...
# Make it so the plot is interactive (zoom/pan/etc.)
tmap_mode("view")

# Plot with this dataframe, color the polygons based on the variable total_positives, and make it translucent
...
census_api_key("9aaca02c1e842e9c82c35fe2f72ba812f35bea62")
...
Adjust the coordinate system to match the other dataset before we join.
```{r}
df_cens_adj=df_cens %>% st_transform(4326)
```

Spatially join the two dataframes...based on the largest amount of intersection between the regions.
```{r}
